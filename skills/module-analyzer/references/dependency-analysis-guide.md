...
所有 subagent 在分析时必须遵守以下规范：

1. **只分析分配的文件**，不越界读取其他子模块的源码
2. **跨模块推断用【待主 agent 验证】标注**——如果在分析中发现对其他子模块的依赖关系，记录推断但不要深入验证。主 agent 在 M6 阶段做交叉验证
3. **深度优先于广度**——宁可把一个核心函数的调用链讲透，也不要对所有函数都浅尝辄止
4. **全局视角**——将子模块放在模块整体语境中分析，解释设计选择如何服务于模块整体职责
5. **叙事衔接**——草稿开头用 1-2 句说明本子模块与前一个子模块的关系，草稿结尾用 1 句铺垫下一个子模块
6. **检验标准**：另一个不熟悉代码的 AI 只读你的分析（不看源码），能否画出这个子模块的架构图并解释工作原理？如果不能，说明缺少了某个关键要素
